...
export type SearchHit = Product & { sectionTitle: string; sectionSlug: string; imgSrc: string };

export const allProducts: SearchHit[] = sections.flatMap((s) =>
  s.products.map((p) => ({
    ...p,
    sectionTitle: s.title,
    sectionSlug: slugify(s.title),
    imgSrc: p.img ?? imgFor(p.query),
  })),
);
